...
sweep_config = {
    ...
    "parameters": {
        ...
        "warmup_frac": {"values": [0.05, 0.10, 0.15]},
        "decay_frac": {"values": [0.40, 0.60, 0.75, 0.85]},
    },
}
...
class DoorKeyProgressReward(gym.Wrapper):
    ...
    def step(self, action):
        ...
        # AGGIUNTA: Salviamo il reward originale nell'info dict per loggarlo
...
def update_q_table(
    ...
    """Aggiorna la Q-Table e restituisce il valore assoluto dell'errore TD."""
    current_q = q_table[state][action]
    max_next_q = float(q_table[next_state].max())

    # Calcolo unificato per ottimizzare
...
def run_loop(cfg, n_episodes: int):
    ...
    for episode in range(n_episodes):
        ...
        step_count = 0
        td_error_sum = (
            0.0  # Ottimizzazione: evitiamo di creare una lista e usare np.mean
        )
        ...
        while not (done or truncated):
            ...
            # Riceviamo e sommiamo l'errore TD
            ...
            ep_raw_reward += info.get("raw_reward", 0.0)  # Recuperiamo il vero reward
            step_count += 1
        ...
        # Log pulito e organizzato.
        # Lo "slash" (/) crea cartelle automatiche in WandB (es. cartella "reward")
        log_data = {
        ...
        # Log dei Q-values ogni 200 step per monitorare divergenza
        ...
        try:
            # Rimosso step=episode, WandB usa il suo step interno in modo pulito
            wandb.log(log_data)
        except Exception:
...
# ──────────────────────────────────────────────────────────────────────────────
# Entrypoint sweep / train / test (Rimangono inalterati)
# ──────────────────────────────────────────────────────────────────────────────
