...
# ──────────────────────────────────────────────────────────────────────────────
# Sweep config (Aggiornato per Step Decay)
# ──────────────────────────────────────────────────────────────────────────────
sweep_config = {
    "method": "bayes",
    ...
    "parameters": {
        ...
        "drop_rate": {"values": [0.3, 0.5, 0.7]},
        "n_steps": {"values": [2, 3, 4, 5, 8]},
    },
}
...
def decrease_epsilon_step(
    ...
    """Decadimento a gradini che si adatta automaticamente al numero totale di episodi."""
    step_size = max(1, n_episodes // n_steps)

    esponente = math.floor(episode / step_size)

    if episode > step_size * (n_steps - 1):
        epsilon = eps_end
    else:
        epsilon = eps_start * (drop_rate**esponente)
...
def run_loop(cfg, n_episodes: int):
    ...
    for episode in range(n_episodes):
        ...
        # Calcolo adattivo del gradino
        epsilon = decrease_epsilon_step(
            episode,
            n_episodes,
            EPS_START,
            getattr(cfg, "eps_end", EPS_END),
            getattr(cfg, "drop_rate", DROP_RATE),
            getattr(cfg, "n_steps", N_STEPS),
        )
...
def test_agent(q_table, n_runs: int = 5, video_folder: str = "./videos"):
    ...
    for i in range(n_runs):
        obs, _ = env.reset()
        state = extract_state(env)  # Passiamo env, come nel training!
        done = False
        truncated = False
        total_reward = 0.0
        ...
        while not (done or truncated):
            ...
            state = extract_state(env)  # Passiamo env
...
if __name__ == "__main__":
    ...
    if args.mode == "sweep":
        ...
        print(
            "Poi esegui: python script.py --mode train --alpha X --gamma Y --drop_rate Z --n_steps W ..."
        )
        ...
        print(
            ...
            f"drop_rate={args.drop_rate}, n_steps={args.n_steps}, eps_end={args.eps_end}, "
