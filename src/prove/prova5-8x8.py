...
sweep_config = {
    ...
    "parameters": {
        ...
        "eps_end": {"values": [0.01, 0.05, 0.10]},
        "shaping_scale": {"values": [0.3, 0.5, 1.0]},
        "eps_exponent": {"values": [2.0, 3.0, 4.0, 5.0]},  # Esponente curva epsilon
    },
}
...
def decrease_epsilon(
    episode: int, n_episodes: int, eps_start: float, eps_end: float, exponent: float
) -> float:
    progress = episode / n_episodes
    epsilon = eps_start - (eps_start - eps_end) * (progress**exponent)
...
def run_loop(cfg, n_episodes: int):
    ...
    for episode in range(n_episodes):
        ...
        epsilon = decrease_epsilon(
            ...
            getattr(cfg, "eps_exponent", EPS_EXPONENT),
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eps_exponent", type=float, default=EPS_EXPONENT)
    ...
    if args.mode == "sweep":
        ...
        print(
            ...
            f"eps_exponent={args.eps_exponent}, eps_end={args.eps_end}, "
            f"shaping_scale={args.shaping_scale}"
        )
        q_table = train_final(
            alpha=args.alpha,
            gamma=args.gamma,
            eps_exponent=args.eps_exponent,
